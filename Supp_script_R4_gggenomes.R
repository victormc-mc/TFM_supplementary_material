...
f <- read_feats(mixedsort(list.files("./renamed_files/", "*.gbk$", full.names=TRUE)))
s <- read_seqs(mixedsort(list.files("./renamed_files/", "*.fa$", full.names=TRUE)))
...
p <- (gggenomes(f_subset, s, feats=list(firmicutes_gc, atts_track, assembly_gap_track, misc_RNA_track), #, , tirs, partial_feature_track 
      ...
      + geom_feat(data=feats(assembly_gap_track), size=5, col="grey", alpha=0.7)
...
ggsave("Firmicutes_Feb_2022_ExplorePipolin.svg", width=15, height=25, limitsize = FALSE)
